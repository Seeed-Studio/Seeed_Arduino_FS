...
#include "SdmmcDiskIo.h"
#include "stm32h7xx_hal.h"
#include "Storage/SdCard/SdmmcStorage.h"
#include "../../FileSystem/FATFS/fatfs/diskio.h"
#include "../../FileSystem/FATFS/fatfs/ffconf.h"
#include "../../FileSystem/FATFS/fatfs/ff.h"
...
__weak uint8_t MX_SDMMC1_SD_Init(void)
{
...
__weak void HAL_SD_MspInit(SD_HandleTypeDef *sdHandle)
{
  ...
  if (sdHandle->Instance == SDMMC1)
  {
    ...
    PC8     ------> SDMMC1_D0
    */
    GPIO_InitStruct.Pin = GPIO_PIN_12 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_9 | GPIO_PIN_8;
...
__weak void HAL_SD_MspDeInit(SD_HandleTypeDef *sdHandle)
{
  ...
  if (sdHandle->Instance == SDMMC1)
  {
    ...
    PC8     ------> SDMMC1_D0
    */
    HAL_GPIO_DeInit(GPIOC, GPIO_PIN_12 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_9 | GPIO_PIN_8);
...
__weak int32_t BSP_SD_Erase(uint32_t Instance, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
__weak int32_t BSP_SD_ReadBlocks_DMA(uint32_t Instance, uint32_t *pData, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
__weak int32_t BSP_SD_WriteBlocks_DMA(uint32_t Instance, uint32_t *pData, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
__weak int32_t BSP_SD_GetCardInfo(uint32_t Instance, HAL_SD_CardInfoTypeDef *CardInfo)
{
  int32_t ret;
...
static seeedfs::SdmmcStorage &sdmmc_storage()
{
  static seeedfs::SdmmcStorage s;
  return s;
}
...
DSTATUS SD_initialize(BYTE pdrv)
{
  ...
  if (sdmmc_storage().begin() && sdmmc_storage().isReady())
  {
    SD_State = 0;
  }
  else
...
{
  (void)pdrv;
  return sdmmc_storage().isReady() ? 0 : STA_NOINIT;
}
...
DRESULT SD_read(BYTE pdrv, BYTE *buff, DWORD sector, UINT count)
{
  ...
  if (!sdmmc_storage().isReady())
  {
    return RES_NOTRDY;
  }

  const uint32_t ss = sdmmc_storage().sectorSize();
  const size_t size = (size_t)count * (size_t)ss;
  return sdmmc_storage().read((uint32_t)sector, 0, buff, size) ? RES_OK : RES_ERROR;
}
...
DRESULT SD_write(BYTE pdrv, const BYTE *buff, DWORD sector, UINT count)
{
  ...
  const uint32_t ss = sdmmc_storage().sectorSize();
  const size_t size = (size_t)count * (size_t)ss;
  return sdmmc_storage().write((uint32_t)sector, 0, buff, size) ? RES_OK : RES_ERROR;
}
...
DRESULT SD_ioctl(BYTE pdrv, BYTE cmd, void *buff)
{
  ...
  {
  case CTRL_SYNC:
    return sdmmc_storage().sync() ? RES_OK : RES_ERROR;
  case GET_SECTOR_COUNT:
    *(DWORD *)buff = (DWORD)sdmmc_storage().sectorCount();
    return RES_OK;
  case GET_SECTOR_SIZE:
    *(WORD *)buff = (WORD)sdmmc_storage().sectorSize();
    return RES_OK;
  case GET_BLOCK_SIZE:
    *(DWORD *)buff = 1;
    return RES_OK;
